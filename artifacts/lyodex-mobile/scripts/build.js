if (process.env.BUILD_MOBILE === "false") {
  console.log("BUILD_MOBILE=false — skipping mobile bundle step");
  process.exit(0);
}
...
if (!workspaceRoot) {
  // No pnpm-workspace.yaml found up the tree — this copy of the mobile app is
  // not the canonical build target. Exit cleanly so the lyodex-replit-upload
  // workspace copy can build without a port-8081 conflict.
  console.log("Skipping mobile build — no pnpm-workspace.yaml found (not the canonical workspace).");
  process.exit(0);
}
...
function killPortProcess(port) {
  try {
    ...
    const result = execSync(`lsof -t -i:${port} 2>/dev/null || true`, {
      encoding: "utf8",
    }).trim();
    if (result) {
    ...
    // lsof not available or no process found — not a fatal error
  }
